...
A Wire is the basic connection element. It is represented as an integer due to the large number of wires on an FPGA. Wires can be hard (not reroutable) connected to other wires, which can be determined through the \texttt{Device} class. %A wire can have a certain direction, however this feature is not used in this work.
...
A \texttt{Potential} can be seen as a set of wires, pins and PIPs which are all connected. %, starting from one pin.
